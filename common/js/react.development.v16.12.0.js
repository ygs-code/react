...
(function (global, factory) {
  ...
  */
  ...
  var BEFORE_SLASH_RE = /^(.*)[\\\/]/;
  // 获取到 index. 文件 和他上一级目录。上一级目录如果一般来说是src
...

  {
    ...
    ReactDebugCurrentFrame.getStackAddendum = function () {
      ...
      if (currentlyValidatingElement) {
        ...
        stack += describeComponentFrame(name, currentlyValidatingElement._source, owner &&
            //获取组件名称根据 type 返回不同的名称
...
// 打印出红色错误日志
  ...
  function createElement(
    ...
    } else if (childrenLength > 1) { // 如果child参数大于1个以上则会把他变成一个数组
      ...
      if (typeof iteratorFn === 'function') {
        {
          ...
          if (iteratorFn === children.entries) {
            !didWarnAboutMaps ? 
            // 打印出红色错误日志
            warning$1(false, 'Using Maps as children is unsupported and will likely yield ' + 'unexpected results. Convert it to a sequence/iterable of keyed ' + 'ReactElements instead.') : void 0;
    ...
    if (calculateChangedBits === undefined) {
    ...
    } else {
      {
        ...
        !(calculateChangedBits === null || typeof calculateChangedBits === 'function') ?
          // 打印出红色错误日志
        warningWithoutStack$1(false, 'createContext: Expected the optional second argument to be a ' + 'function. Instead received: %s', calculateChangedBits): void 0;
      }
    }
      ...
      if (render != null && render.$$typeof === REACT_MEMO_TYPE) {
        ...
        !( // Do not warn for 0 arguments because it could be due to usage of the 'arguments' object
          ...
          render.length === 0 || render.length === 2) ?
            // 打印出红色错误日志
          warningWithoutStack$1(false, 'forwardRef render functions accept exactly two parameters: props and ref. %s', render.length === 1 ? 'Did you forget to use the ref parameter?' : 'Any additional parameter will be undefined.'): void 0;
      }

      if (render != null) {
        !(render.defaultProps == null && render.propTypes == null) ? 
          // 打印出红色错误日志
          warningWithoutStack$1(false, 'forwardRef render functions do not support propTypes or defaultProps. ' + 'Did you accidentally pass a React component?'): void 0;
      }
    }
  ...
  function useContext(Context, unstable_observedBits) {
...

    {
      !(unstable_observedBits === undefined) ? 
        //打印出红色错误日志
      warning$1(false, 'useContext() second argument is reserved for future ' + 'use in React. Passing it is not supported. ' + 'You passed: %s.%s', unstable_observedBits, typeof unstable_observedBits === 'number' && Array.isArray(arguments[2]) ? '\n\nDid you call array.map(useContext)? ' + 'Calling Hooks inside a loop is not supported. ' + 'Learn more at https://fb.me/rules-of-hooks' : ''): void 0; // TODO: add a more generic warning for invalid values.
...
//获取组件名称根据 type 返回不同的名称
    ...
    if (typeof type.getDefaultProps === 'function') {
      !type.getDefaultProps.isReactClassApproved ?
        // 打印出红色错误日志
      warningWithoutStack$1(false, 'getDefaultProps is only used on classic React.createClass ' + 'definitions. Use a static property named `defaultProps` instead.') : void 0;
    }
  }
  /**
