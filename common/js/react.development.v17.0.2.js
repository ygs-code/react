...
*现代基类。相反，我们定义一个getter来警告它是否被访问。
  */
  ...
  // 保留的 属性 用于 用户在 props，和state 中不能使用该属性
  var RESERVED_PROPS = {
    key: true,
    ref: true,
    __self: true,
  ...
  function defineKeyPropWarningGetter(props, displayName) {
    ...
    warnAboutAccessingKey.isReactWarning = true; //  标志是react 不可以读取
...
Children.defaultProps = {
  ...
  */
  ...
  //forwardRef 判断 错误日志 判断 render 是否正确
  ...
  // 检测typeSpecs 的 方法 是不是函数 如果不是则发出错误警告，并且标签函数
  function checkPropTypes(
    typeSpecs, //typeSpecs 对象里面是方法
    values, // 值
    location, // 地址路径
    componentName, // 组件名称
    element // vonde 或者真实dom
  ) {
    {
  ...
  if (
    ...
    //使用setTimeout实现。
    typeof window === "undefined" || // Check if MessageChannel is supported, too.  //检查是否也支持MessageChannel。
  ...
  // TODO: Use symbols?
  var ImmediatePriority = 1;  // 当务之急优先级最高
  var UserBlockingPriority = 2; // // 用户阻塞级别的优先级
  var NormalPriority = 3;  // 正常的优先级
  var LowPriority = 4; // 较低的优先级
  var IdlePriority = 5;  // 优先级最低，表示任务可以闲置
  ...
  var maxSigned31BitInt = 1073741823; // Times out immediately 次立即

  var IMMEDIATE_PRIORITY_TIMEOUT = -1; // Eventually times out 最后次

  var USER_BLOCKING_PRIORITY_TIMEOUT = 250; //用户阻塞优先级超时
  var NORMAL_PRIORITY_TIMEOUT = 5000; //正常优先级超时
  var LOW_PRIORITY_TIMEOUT = 10000; // Never times out 从来没有超时

  var IDLE_PRIORITY_TIMEOUT = maxSigned31BitInt; // Tasks are stored on a min heap 任务存储在一个最小堆上
  ...
  var Scheduler = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    unstable_ImmediatePriority: ImmediatePriority, // 当务之急调度器，优先级最高
