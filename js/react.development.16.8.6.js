...
(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() :  // 判断是否在node环境或者webpack环境
    typeof define === 'function' && define.amd ? define(factory) :  // amd 环境
      (global.React = factory()); // cdn 环境
}(this, (function () {
  'use strict';
  ...
  // nor polyfill, then a plain number is used for performance. 也不是填充，然后一个普通的数字是用于性能。
  ...
  */
  ...
  //检测老版本V8中有bug的属性for, map,forEach枚举顺序。 如果枚举顺序错误则返回false
   ...
   */
  // 判断变量形参如果是 如果参数是undefined 则报错
  var validateFormat = function () { };

  {
    //不变量需要错误消息参数 方法 如果参数是undefined 则报错
...

  {
    var printWarning = function (format) {
      ...
      for (
        var _len = arguments.length,  // 函数形参长度
        args = Array(_len > 1 ? _len - 1 : 0),   // 函数形参长度大于1么，如果大于1 则为 长度-1，否则 为0
        _key = 1;  // key 等于1
        _key < _len;  // 当key
        _key++
      ) {
    ...
    lowPriorityWarning = function (condition, format) {
      ...
      if (!condition) {
        for (
          var _len2 = arguments.length,  // 参数长度
          args = Array(_len2 > 2 ? _len2 - 2 : 0), // 如果参数长度大于2 那么就等于 参数长度-2，否则等于0
          _key2 = 2; // key 等于 2
          _key2 < _len2;  // 如果key 小于 len 长度
          _key2++  // key++
        ) {
          console.log('_key2', _key2)
          // 这里就是等参数的长度需要大于4 才能开始进入该循环
          args[_key2 - 2] = arguments[_key2];
        }
        console.log('args', args)
        console.log('format', format)
        console.log('[format].concat(args)', [format].concat(args))
...

  {
    warningWithoutStack = function (condition, format) {
      ...
      if (args.length > 8) {
        // Check before the condition to catch violations early.  如果参数大于8 则报错
        throw new Error('warningWithoutStack() currently supports at most 8 arguments.');
      }
      // 如果condition 存在 则不会报错
      if (condition) {
        return;
      }
