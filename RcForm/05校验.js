const createForm = (options) => {
  return (Component) => {
    ...
    class FieldsStore {
      ...
      // 获取校验字段
      getHasRulesFields(fields) {
        var hasRulesFields = fields.filter((key) => {
          if (
            this.fieldsMeta[key].rules &&
            this.fieldsMeta[key].rules.length >= 1
          ) {
            for (let item of this.fieldsMeta[key].rules) {
        ...
        return hasRulesFields.reduce((acc, key) => {
          // acc = {
          //   ...acc,
          //   [key]: this.fieldsMeta[key],
          // };
          return _.set(acc, key, this.fieldsMeta[key]);
          // return acc;
        }, {});
      }
      // 校验规则
      checkRules(fields) {
        var hasRulesFields = this.getHasRulesFields(fields);

        const errors = Object.keys(hasRulesFields).reduce((acc, nextKey) => {
          let meta = this.getFieldMeta(nextKey);
          let value = this.getFieldValue(nextKey);
          let fieldErrors = _.get(acc, nextKey) || {};
          let errors = fieldErrors.errors || [];
          for (let item of hasRulesFields[nextKey].rules) {
            let errorInfo = null;
            let callbackeErorInfo = null;
            if (item.required) {
              if (!(value && value.toString().trim())) {
                ...
                _.set(acc, nextKey, errors);
              }
            }
            if (item.validator) {
              errorInfo = item.validator(meta.rules, value, (error) => {
                callbackeErorInfo = error;
              });
              if (errorInfo || callbackeErorInfo) {
                errors.push({
                  field: nextKey,
                  message: errorInfo || callbackeErorInfo,
                });
                _.set(acc, nextKey, errors);
              }
            }
          }
          this.setFieldMeta(nextKey, {
            ...meta,
            errors: errors,
          });
          console.log("acc=====", acc);
        ...
        console.log("hasRulesFields=", hasRulesFields);
    ...
    return class Form extends React.Component {
      ...
      getAllValues() {
        ...
        return Object.keys(fieldsMeta).reduce(function (acc, name) {
          // 获取字段的值
          return _.set(acc, name, this.getValueFromFields(name, fields));
      ...
      getForm() {
        return {
          ...
          validateFields: (callback) => {
            ...
            var fieldNames = this.fieldsStore.getValidFieldsName();
            console.log("this.fieldsStore=", this.fieldsStore);
            // this.fieldsStore.getHasRulesFields(fieldNames);
            console.log(" this.fieldsStore=", this.fieldsStore);
            // console.log(
            //   "this.fieldsStore.getHasRulesFields=========",
            //   this.fieldsStore.getHasRulesFields(fieldNames)
            // );
            this.fieldsStore.checkRules(fieldNames);
            // 获取字段值
